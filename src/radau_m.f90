...
public::Int_Radau  !Gauss Radau 15 (Everhart 1974, 1985)
public::Int_Radau_rel
public::Int_Radau_yrel
 contains

!***************************************************************************
subroutine Int_Radau(p,rv,mass,add)
...
integer(kind=intdble)::order(1:p%NrKoerper),stiffcnt
...
call radau_step(rv,mass,totbody,body,cutbody,m0cutbody,dtk,dtdid,p%eps,rho,c_b, &
...
! 
!------------------------------------------------------------------------------
      subroutine radau_step(rv,mass,totbody,body,cutbody,m0cutbody,& 
...
! Calculate forces at the start of the sequence
      call frho(rv,totbody,bmc,bp1,totm0,mass,acc,rho) 
!
...
!$omp shared(g,b,d) &
!$omp firstprivate(bmc,bp1,totm0) &
!$omp private(k)
!$omp do
      do  k= 1, bmc
...
!$omp firstprivate(bmc,bp1,totm0,t) &
!$omp private(s,k)
...
! Calculate forces at the current substep
          call frho(rv1,totbody,bmc,bp1,totm0,mass,acc1,rho) 
!         call frho(rv,totbody,bmc,bp1,totm0,mass,acc,rho) 
...
!#################################################################
subroutine Int_Radau_rel(p,rv,mass,add)
...
! 
call radau_rel_step(rv,mass,totbody,body,cutbody,m0cutbody,dtk,dtdid,p%eps,rho,c_b, &
                c_e,c_h,c_xc,c_vc,c_c,c_d,c_r)
!
   !t=t+dtdid/k
   kahan_y=dtdid/k-kahan_c
...
! 
!------------------------------------------------------------------------------
      subroutine radau_rel_step(rv,mass,totbody,body,cutbody,m0cutbody,& 
                            t,tdid,tol,rho,b,e,h,xc,vc,c,d,r)
...
!
! Calculate forces at the start of the sequence
      call frho_rel(rv,totbody,bmc,bp1,totm0,mass,acc,rho) 
!
! Find G values from B values predicted at the last call (Eqs. 7 of Everhart)
!
!massive
...
!
! Calculate forces at the current substep
          call frho_rel(rv1,totbody,bmc,bp1,totm0,mass,acc1,rho) 
! Update G values using Eqs. 4 of Everhart, and update B values using Eqs. 5
...
!$omp do
   do i=1,bmc
      ...
      do j=bp1,totm0 !massive on massless
          force(i,:)=force(i,:)-mass(j)*rij(j,i,:)/d(j,i)**3._double
      end do
...
!massless: accelerations on massive
!$omp do
   do i=bp1,totm0
      do j=1,bmc
...
!#################################################################
subroutine frho_rel(rv,totbody,bmc,bp1,totm0,mass,frel,d) 
implicit none
integer(kind=intdble)::totbody,bmc,bp1,totm0,i,j,l
real(kind=double)::rv(1:totbody,1:6),mass(1:totbody)
real(kind=double)::frel(1:totbody,1:3),cm2,k2,drv(1,4:6)
...
!non-relativistic accelerations
!$omp do
 do i=1,bmc
      do j=1,bmc!massive on massive
         if(i.eq.j) then
            else
   frel(i,1:3)=frel(i,1:3)+mass(j)*rij(i,j,1:3)/d(i,j)**3 
   !relativistic two body problem with the sun           
         end if
        end do !j
   
   do j=bp1,totm0!massless on massive
     frel(i,1:3)=frel(i,1:3)-mass(j)*rij(j,i,1:3)/d(j,i)**3 
   end do !j 
 end do !i
!$omp end do nowait
...
!relativistic two body accelerations due to the Sun  
!$omp do
 do i=2,bmc
! dummy variable delta v sun particle
 drv(1,4:6)=rv(i,4:6)-rv(1,4:6)
 frel(i,1:3)=frel(i,1:3)-cm2*k2/d(i,1)**3*mass(1)*(rij(i,1,1:3)*(4._double*mass(1)/d(i,1)-Dot_Product(drv(1,4:6),drv(1,4:6))) + &
             4._double*Dot_Product(rij(i,1,1:3),drv(1,4:6))*drv(1,4:6))
...
return
end subroutine frho_rel


!******************************************************************

subroutine Int_Radau_yrel(p,rv,mass,add)
...
! 
call radau_yrel_step(rv,mass,totbody,body,cutbody,m0cutbody,dtk,dtdid,p%eps,rho,c_b, &
                c_e,c_h,c_xc,c_vc,c_c,c_d,c_r)
!
   t=t+dtdid/k
...
! 
!------------------------------------------------------------------------------
      subroutine radau_yrel_step(rv,mass,totbody,body,cutbody,m0cutbody,& 
                            t,tdid,tol,rho,b,e,h,xc,vc,c,d,r)
...
!
! Calculate forces at the start of the sequence
      call frho_yrel(rv,totbody,bmc,bp1,totm0,mass,acc,rho) 
!
! Find G values from B values predicted at the last call (Eqs. 7 of Everhart)
!
!massive
...
!
!$omp parallel default(NONE) &
!$omp shared(h,rv1,rv,b,acc) &
!$omp firstprivate(bmc,bp1,totm0,t,tol,iter,j) &
!$omp private(s,k)
! Calculate position predictors using Eqn. 9 of Everhart
          s(1) = t * h(j)
          s(2) = s(1) * s(1) * .5_double
...
!
! Calculate forces at the current substep
          call frho_yrel(rv1,totbody,bmc,bp1,totm0,mass,acc1,rho) 
! Update G values using Eqs. 4 of Everhart, and update B values using Eqs. 5
...
!$omp parallel default(NONE) &
!$omp shared(rv,xc,vc,b,acc) &
!$omp firstprivate(temp,tdid,bp1,totm0,bmc) & 
!$omp private(k)
!massive
!$omp do
      do k = 1 , bmc
...
!******************************************************************************************
subroutine frho_yrel(rv,totbody,bmc,bp1,totm0,mass,frel,d) 
!Spheric PPN field around Sun -> only two body relativity included!
use global_m
use yark_m
use transform_m
implicit none
integer(kind=intdble)::totbody,bmc,bp1,totm0,i,j,l
real(kind=double)::rv(1:totbody,1:6),mass(1:totbody)
real(kind=double)::frel(1:totbody,1:3),cm2,k2
real(kind=double),dimension(1:totbody,1:bmc,1:3)::rij
